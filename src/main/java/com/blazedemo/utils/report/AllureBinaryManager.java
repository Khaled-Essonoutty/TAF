...
public class AllureBinaryManager {
    ...
    public static void downloadAndExtract() {
        try {
            ...
            if (Files.exists(extractionDir)) {
                LogsUtil.info("Allure binaries already exist.");
                return;
            }
            ...
            LogsUtil.info("Allure binaries downloaded and extracted.");
            // Give execute permissions to the binary if not on Windows
            if (!OSUtils.getCurrentOS().equals(OSUtils.OS.WINDOWS)) {
            ...
            LogsUtil.error("Error downloading or extracting binaries", e.getMessage());
        }
    }
        ...
        try {
            ...
            if (!Files.exists(zipFile)) {
                ...
                try (BufferedInputStream in = new BufferedInputStream(new URI(url).toURL().openStream());
                    ...
                    LogsUtil.error("Invalid URL for Allure download: ", e.getMessage());
                }
            }
            return zipFile;
        } catch (Exception e) {
            LogsUtil.error("Error downloading Allure zip file", e.getMessage());
            return Paths.get("");
        }
    ...
    private static void extractZip(Path zipPath)   {
        try (ZipInputStream zipInputStream = new ZipInputStream(Files.newInputStream(zipPath))) {
            ...
            LogsUtil.error("Error extracting Allure zip file", e.getMessage());
        }
    }
}
