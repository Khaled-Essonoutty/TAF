    ...
    public static void startRecording() {
        if (getPropertyValue("recordTests").equalsIgnoreCase("true")) {
            try {
                ...
                if (getPropertyValue("executionType").equalsIgnoreCase("local")) {
                    recorder.set(RecorderFactory.getRecorder(VideoRecorder.conf().recorderType()));
                    // Start recording
                    recorder.get().start();
                    LogsUtil.info("Recording Started");

                }


            } catch (Exception e) {
                LogsUtil.error("Failed to start recording: " + e.getMessage());
            }
    ...
    public static void stopRecording(String testMethodName) {
        try {
            if (recorder.get() != null) {
                ...
                LogsUtil.info("Video file saved at: " + videoFile.getAbsolutePath());


                // Convert the video to .mp4 format
                File mp4File = encodeRecording(videoFile);
                LogsUtil.info("Recording Stopped and Converted to MP4: " + mp4File.getName());
            }
        } catch (Exception e) {
            LogsUtil.error("Failed to stop recording: " + e.getMessage());
        } finally {
            recorder.remove();
        }
    }
    ...
    private static File encodeRecording(File sourceFile) {
        ...
        try {
            ...
            if (targetFile.exists()) {
                sourceFile.delete();
                LogsUtil.info("Deleted original AVI file: " + sourceFile.getName());
            }
        } catch (EncoderException e) {
            LogsUtil.error("Failed to convert video to MP4: " + e.getMessage());
        }
        return targetFile;
    }
}
